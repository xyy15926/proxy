...
-	耗时相差不大的job队列总是比较好处理，难以调度的job是
	-	耗时相差大
	-	IOjob、计算job混合

-	内核调度除考虑job（线程）外，还会考虑进程因素
...
先进先出：按job在队列中的顺序依次调用，执行完job再执行下个
job，仅在job结束后才会切换job
...
###	Shortest Job First/Shortest Remained Time Job

最短耗时job优先：有限调度耗时短job

-	优点
	-	平均相应时间短：长耗时job不断推移，必然统计出较短
		平均响应时间
...
时间片轮转：给队列中每个job时间片，时间片结束之后job移至
队列末尾，切换到执行下个job

-	优点
	-	每个job可以得到公平调度
	-	耗时短job即使在耗时长job之后也可以较快得到执行

-	缺点
	-	job切换引起的调度开销大，需要多次切换job上下文
	-	时间片不好设置
		-	时间片足够小则退化为SFJ
		-	时间片足够大则退化为FIFO
	-	需要知道job（剩余）执行时间
...
多级反馈队列：监控job处理耗时，若job未用尽分配资源则提高
优先级，否则降低其优先级
...
-	具体方案
	-	job、分片时长具有相对应的不同优先级
		...
		-	高级优先级job可以抢占低优先级job
		-	新job位于高优先级job
	-	同一优先级job使用*Round Robin*
		（事实上仅有最低优先级job使用*Round Robin*算法，
		其他优先级都是FIFO）
		-	时间片用完后job结束则正常退出系统，否则优先级
			下滑一等级
...
-	多核场景
	-	应该为每个CPU分配单独MFQ，同时采用
		*Affinity Scheduling*保证job尽量同一CPU核上执行，
		避免CPU cache频繁失效
	-	若共用MFQ，则容易出现
		-	随CPU增长，对MFQ锁争抢严重
		-	job每次执行位于的CPU不同，CPU Cache需要在不同
			CPU间转移
