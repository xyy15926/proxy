...
-	当$(X, Y)$为二元正态分布且不相关时，$t$服从自由度为$n-2$
	的t分布
...
> - $Rank(X), Rank(Y)$：变量$X, Y$的秩（应同序）
	（相同值秩取均值）
> - $d_i$：变量对$X, Y$中，二者秩差值

-	Spearman秩相关系数被定义为变量秩的Pearson相关系数

> - Spearman秩相关系数也可以使用Fisher变换检验显著性
...
-	$\tau_c$适合“层级”数据，即两个变量取值类似划分、内部细分
	...
	|III-2|0|0|30|
	|III-2|0|0|30|
	-	对以上数据，$\tau_b$取值在0.9附近，而$\tau_c$取1

> - 有序对：对$(X_i, Y_i), (X_j, Y_j)$，满足
	$X_i < X_j, Y_i < Y_j$或
	$X_i > X_j,Y_i > Y_j$则为有序对
> - 无序对：对$(X_i, Y_i), (X_j, Y_j)$，满足
	$X_i < X_j, Y_i > Y_j$或
	$X_i > X_j, Y_i < Y_j$则为无序对
