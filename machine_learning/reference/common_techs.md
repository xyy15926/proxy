...
$$\begin{align*}
\leftarrow x_i & = (\frac {x_i^{(1)}} {\L_p(\leftarrw x_i)},
	\frac {x_i^{(2)}} {\L_p(\leftarrw x_i)}, \cdots,
	\frac {x_i^{(d)}} {\L_p(\leftarrw x_i)})^T \\
L_p(\leftarrow x_i) & = (|x_i^{(1)}|^p + |x_i^{(2)}|^p + 
	\cdots + |x_i^{(d)}^p)^{1/p}
\end{align*}$$
